...
class ExternalCronController {
    ...
    /**
     * Simple validation - IP + User-Agent
     */
    private function is_valid_cronjoborg_request() {
        // Check User-Agent first (simplest)
        $user_agent = $_SERVER['HTTP_USER_AGENT'] ?? '';
        if (strpos($user_agent, 'cron-job.org') === false) {
            error_log('UJC: Invalid User-Agent: ' . $user_agent);
            return false;
        }
        
        // Check IP against known cron-job.org IPs
        $client_ip = $this->get_client_ip();
        if (!$this->is_cronjoborg_ip($client_ip)) {
            error_log('UJC: Invalid IP: ' . $client_ip);
            return false;
        }
    ...
    /**
     * Check if IP is from cron-job.org
     */
    private function is_cronjoborg_ip($client_ip) {
        // Get cached IP list or fetch fresh
        $cached_ips = get_transient('ujc_cronjoborg_ips');
        
        if (!$cached_ips) {
            $result = $this->repository->getExecutorNodes();
            
            if ($result['success']) {
                $cached_ips = $result['ips'];
                set_transient('ujc_cronjoborg_ips', $cached_ips, HOUR_IN_SECONDS); // Cache for 1 hour
            } else {
                // Fallback to known IP if API fails
                $cached_ips = ['91.99.23.109']; // Known cron-job.org IP
                set_transient('ujc_cronjoborg_ips', $cached_ips, 300); // Cache for 5 minutes only
            }
        }
        
        return in_array($client_ip, $cached_ips);
    }
