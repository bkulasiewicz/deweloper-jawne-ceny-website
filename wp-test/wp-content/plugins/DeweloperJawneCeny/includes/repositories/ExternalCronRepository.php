...
class ExternalCronRepository {
    ...
    /**
     * Get executor nodes (IP addresses) for validation
     * 
     * @return array ['success' => bool, 'ips' => array, 'message' => string]
     */
    public function getExecutorNodes() {
        $response = wp_remote_get(self::API_BASE_URL . '/executor-nodes.json', [
            'timeout' => 10
        ]);
        ...
        if ($response_code === 200) {
            ...
            if ($data && isset($data['ipAddresses'])) {
                return [
                    'success' => true,
        ...
        return [
            'success' => false,
            'ips' => [],
            'message' => 'Failed to retrieve IP addresses: ' . wp_remote_retrieve_body($response)
        ];
    }
    ...
    public function getAvailableSchedules() {
        return [
            ...
            '24hour' => ['hours' => [6], 'minutes' => [0], 'interval_text' => 'Codziennie o 6:00 UTC']
        ];
    }
}